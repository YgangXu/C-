...
void SpeechManager::speechContest()
{
    ...
    for(vector<int>::iterator it=v_Src.begin();it!=v_Src.end();it++)
    {
        ...
        for(int i=0;i<10;i++)
        {
            double score=(rand()%401+600)/10.f;  //600-1000之间
//            cout<<score<<" ";
            d.push_back(score);
        }
//        cout<<endl;
        ...
        double avg=(double )sum/d.size();//平均分
        //打印平均分
...
{
    ofstream ofs;
    ofs.open("speech.csv",ios::out|ios::app);//用追加的方式写入
    //将每个选手的数据写入到文件中
...
//清空文件
void SpeechManager::cleanRecord()//加上作用域相当于成员函数，否则就是全局函数
{
    cout<<"是否确定清空文件？"<<endl;
    cout<<"1、是"<<endl;
    cout<<"2、否"<<endl;
    ...
    {
        //确认清空
        ofstream ofs("speech.csv",ios::trunc);//相当于打开文件清空，不存在就创建空文件
        ofs.close();
